        ...
        <div class="box">
            <form action="{{ route($namespace_update, $item->_id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="box-body">
                    ...
                    <div class="form-group row">
                        <label class="col-form-label col-md-2">Тип</label>
                        <select name="type" class="selectpicker col-md-10" required>
                            <option value="">Выберите тип</option>
                            @foreach($category_types as $category_type)
                                <option @if($item->type && $item->type->sys_name == $category_type->sys_name) selected @endif
                                        value="{{ $category_type->sys_name }}">{{ $category_type->name }} - {{ $category_type->sys_name }}</option>
                            @endforeach
                        </select>
                    </div>
