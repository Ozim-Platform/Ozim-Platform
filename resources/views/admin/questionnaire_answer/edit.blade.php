                ...
                <div class="box-body">
                    ...
                    @foreach($item->questionnaire->questions ?? [] as $index => $question)
                        <div class="form-group row">
                            ...
                            <div class="col-md-10">
                                <span>{{ $question }}</span>
                                <input @if($item->answers[$index]) checked @endif type="radio" id="yes_{{$index}}"
                                       name="answers{{$index}}" value="1">
                                <label for="yes_{{$index}}">Да</label>

                                <input @if(!$item->answers[$index]) checked @endif type="radio" id="no_{{$index}}"
                                       name="answers{{$index}}" value="0">
                                <label for="no_{{$index}}">Нет</label>
                            </div>
                        </div>
                    @endforeach
